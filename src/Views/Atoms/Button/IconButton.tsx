...
const IconButton: React.FC<IconButtonModel> = ({
  ...
  const styles = {
    variantClass: `text-3xl cursor-pointer hover:bg-opacity-80 rounded-full inline-block p-1 ${variant === "contained" ? `${getBackgroundColor()} text-white` : variant === "outlined" ? `border ${getBorderColor()} ` : ''}`,
    importedStyles: useAtomStyle({ size, color, margin, padding, className, elevation, align, weight }),
  }
  ...
  return (<button onClick={onClick}><Myicon className={`${Object.values(styles).join(" ")}`} /></button>);
